...
def forward_kinematics(rotmats, parents, offsets):
    """
    convert rotation matrix to joint positions based on
    provided skeleton structure using forward kinematics
    
    adapted from 
    https://github.com/wei-mao-2019/LearnTrajDep/blob/master/utils/forward_kinematics.py#L219
    and
    https://github.com/una-dinosauria/human-motion-prediction/blob/master/src/forward_kinematics.py#L14
    
    rotmats: rotation matrix data [... num_joints, 3, 3]
    parents: joint parent indices
    offsets: offsets between joints [num_joints, 3]
    
    return: joint positions [..., num_joints, 3]

    """
    assert rotmats.shape[-1] == 3 and rotmats.shape[-2] == 3
    assert offsets.shape[-1] == 3
    rotmats_copy = rotmats.copy()
    ori_shape = rotmats_copy.shape[:-3]
    num_joints = rotmats_copy.shape[-3]
    p = np.tile(offsets, ori_shape + (1, 1))
    for i in np.arange(1, num_joints):
        if parents[i] >= 0:
            rotmats_copy[..., i, :, :] = np.matmul(rotmats_copy[..., i, :, :], rotmats_copy[..., parents[i], :, :])
            p[..., i, :] = np.matmul(offsets[i], rotmats_copy[..., parents[i], :, :]) + p[..., parents[i], :]
    return p
